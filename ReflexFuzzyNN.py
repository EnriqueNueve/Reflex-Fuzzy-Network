...
class ReflexFuzzyNeuroNetwork(object):
    ...
    def query(self, pt, validation=False):
        ...
        return np.round(np.array(CLN_max_values) + np.array(Compensation_Values),4)
...
def main():
    ...
    X = np.array([[0,0],[.3,.3],[.2,.2],[.6,.6],[.8,.8],[.9,.9],[.82,.82],[.88,.88]]).T
    Y = np.array([1,1,2,2,2,2,1,1])

    # --- Declare network --- #
    nn = ReflexFuzzyNeuroNetwork(gamma=2, theta=.4)
    ...
    # --- Query point --- #
    print("Query for [.85,.85] : ", nn.query([.85,.85]))
    print("Query for [.2,.2] : ", nn.query([.2,.2]))
    print("Query for [.5,.5] : ", nn.query([.5,.5]))
